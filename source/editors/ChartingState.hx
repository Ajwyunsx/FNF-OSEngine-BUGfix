	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				if (FlxG.keys.pressed.SHIFT #if android || virtualPad.buttonY.justPressed #end )
					resetSection(true);
				else
					resetSection();
			}
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if android || virtualPad.buttonUp.justPressed || virtualPad.buttonDown.justPressed #end)
			{
				FlxG.sound.music.pause();

				var holdingShift:Float = 1;
				if (FlxG.keys.pressed.CONTROL) holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT #if android || virtualPad.buttonY.justPressed #end) holdingShift = 4;
			...
			if (FlxG.keys.justPressed.D #if android || _virtualpad.buttonRight.justPressed #end)
				changeSection(curSec + shiftThing);ss
